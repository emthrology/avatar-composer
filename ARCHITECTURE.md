...
- [x] 외부 authored GLB 파츠 로딩 — `partLoader.ts` `loadPart()`(자체 스켈레톤 → base rebind). 실측 스탠드인(`scripts/extractTops.mjs` → male_sample 에서 Tops 추출 → `Tops_sample.glb`, `PART_TEST_URL`)로 **시각 검증 통과**: 외부 GLB 로드 + 본 56개 0 누락 rebind + 팔 흔들기 변형 추종 확인. ⚠️ 스탠드인은 base 보다 **균일 2.5cm 큼** → 옷·발이 그만큼 떠 보임(파일 간 빌림 인공물, loadPart 버그 아님; 규약 준수 파츠는 오프셋 0). 이 오프셋이 ASSET_SPEC §1 신장/스케일 락(1.756m)이 하중을 받는다는 실증.
