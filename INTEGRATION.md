...
- **다중 베이스 예정**: 현재 남자1만. 성숙하면 남자2/여자1 추가 → 함께 통합.
...
5. **베이스 불가지 유지 → 캐릭터 축.** 다중 베이스가 붙으면 컨벤션 락·파츠 라이브러리가 **베이스별**이 되고
   파츠는 베이스에 종속(남자1 셔츠 ≠ 여자1 셔츠). 엔진은 이미 `load*(url, baseVrm)`로 베이스 불가지 —
   새 기능도 baseVrm/스펙을 **파라미터로** 받게 유지. 남자2 도착 시 `constants`를 `characters[]`로 승격.
...
| `MODULE_PARTS` 레지스트리 + 컨벤션 락(`constants.ts`) | **엔진** | 이관 (단일 베이스 가정은 캐릭터 축으로 승격) |
| 파츠 가시성/장착 토글, `setEyeColor`/`setVisible`/`sync` | **엔진 API** | 유지 — drei 에디터 UI가 바인딩 |
| 유휴 시선(범위 내 랜덤 드리프트) | **정책** | drei의 시선 구현으로 교체 |
| `ComposerScene` 디버그 패널 · `PartsPanel` · 눈색 스와치 UI | **스캐폴딩** | 폐기 — drei 에디터 탭 UI가 대체 |
| 더미 ①②③(`dummyParts.ts`, `DUMMY_*`) · wave 토글 | **스캐폴딩** | 폐기 (PoC 검증용) |
| 단일 베이스 가정(`BASE_URL`/`BASE_SPEC` 단일) | **엔진(미성숙)** | 캐릭터 축(`characters[]`)으로 승격 |
